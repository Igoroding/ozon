(function() {
  ...
  const css = `
    ...
    .sh-logo { width: 44px; height: 44px; background: #005bff; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 900; color: #fff; box-shadow: 0 0 24px rgba(0,91,255,0.3); flex-shrink: 0; letter-spacing: 0; }
  ...
  header.innerHTML = `
    <div class="site-header-top">
      <div class="sh-logo">OZON</div>
      <div>
        <div class="sh-name">Ozon Analytics</div>
        <div class="sh-sub">Стольников</div>
      </div>
    </div>
    <nav class="site-nav">
      <a href="ozon_rnp.html"    ${isRnP    ? 'class="active"' : ''}>📈 РнП</a>
      <a href="ozon_stocks.html" ${isStocks ? 'class="active"' : ''}>📦 Остатки</a>
    </nav>`;
